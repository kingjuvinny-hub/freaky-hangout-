    ...
    <div className="space-y-6 text-gray-300 max-h-[70vh] overflow-y-auto pr-2">
      <div>
        ...
        <div className="flex gap-2">
            <button onClick={() => dispatch({type: 'UPDATE_SETTINGS', payload: {theme: 'neon'}})} className={`flex-1 p-2 rounded-md font-bold transition-colors ${settings.theme === 'neon' ? 'bg-pink-600 text-white' : 'bg-black/20'}`}>Neon Arcade</button>
            <button onClick={() => dispatch({type: 'UPDATE_SETTINGS', payload: {theme: 'spooky'}})} className={`flex-1 p-2 rounded-md font-bold transition-colors ${settings.theme === 'spooky' ? 'bg-orange-600 text-white' : 'bg-black/20'}`}>Spooky</button>
        </div>
      </div>
