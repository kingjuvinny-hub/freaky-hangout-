...
export const gameReducer = (state: GameState, action: Action): GameState => {
  switch (action.type) {
    case 'SET_STATE':
        return { ...action.payload, playerId: state.playerId }; // Keep local playerId

    case 'CREATE_ROOM': {
        const { player, roomId } = action.payload;
        const newPlayer: Player = { ...player, score: 0, streak: 0 };
        return {
            ...createInitialState(),
    ...
    case 'ADD_PLAYER': {
      ...
      const newPlayer: Player = {
        id: action.payload.id || `player_${Date.now()}`,
    ...
    case 'RESET_GAME': {
        if(state.roomId) { // Online game reset
            // Go back to lobby, keep players
            const resetPlayers = state.players.map(p => ({...p, score: 0, streak: 0}));
            return {
                ...createInitialState(),
                roomId: state.roomId,
                hostId: state.hostId,
...
const GameContext = createContext<{ state: GameState; dispatch: (action: Action) => void } | undefined>(undefined);
...
export const GameProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const dispatchAction = (action: Action) => {
    if (state.roomId) {
        // Online mode: send action to the service
        multiplayerService.sendAction(action);
    } else {
        // Offline mode: dispatch locally
        dispatch(action);
    }
  };

  useEffect(() => {
    multiplayerService.initialize((newState) => {
        dispatch({ type: 'SET_STATE', payload: newState });
    });
    
    const reconnectedState = multiplayerService.reconnect();
    if(reconnectedState){
        dispatch({ type: 'SET_STATE', payload: reconnectedState });
    }
  }, []);
  ...
  useEffect(() => {
    loadCustomPacks();
    loadLifetimeStats();
  }, [loadCustomPacks, loadLifetimeStats]);
  ...
  return (
    <GameContext.Provider value={{ state, dispatch: dispatchAction }}>
      {children}
