...
const GameScreen: React.FC = () => {
    ...
    const isMyTurn = !state.roomId || state.playerId === currentPlayer?.id;

    useEffect(() => {
        if (!state.currentTurn && state.gamePhase === GamePhase.PLAYING && isMyTurn) {
    ...
    }, [state.currentTurn, state.gamePhase, dispatch, isMyTurn, isDoubleDown]);
    ...
    return (
        <div className="flex flex-col items-center justify-center space-y-6 animate-fade-in">
            ...
            <div className="w-full max-w-2xl text-center">
                <p className="text-xl text-gray-400">{isMyTurn ? "It's your turn," : `It's ${currentPlayer.name}'s turn,`}</p>
                <div className="flex items-center justify-center gap-4 mt-1">
                    <span className="text-4xl sm:text-5xl">{currentPlayer.avatar}</span>
                    <h1 className="text-4xl sm:text-5xl font-black text-[var(--color-secondary)] drop-shadow-[0_0_10px_var(--color-secondary)]">{currentPlayer.name}</h1>
                </div>
            </div>
                ...
                {showDareReveal ? (
                    <div className="space-y-6">
                        <div className="animate-pulse flex flex-col items-center justify-center space-y-4 text-gray-400">
                            <Spinner />
                            <h2 className="text-2xl font-bold">{isMyTurn ? "Thinking of a dare..." : "A dare is being chosen..."}</h2>
                        </div>
                        {isMyTurn && (
            ...
            {state.currentTurn && (
                <div className="flex flex-col sm:flex-row gap-4 w-full max-w-2xl animate-fade-in">
                    <button onClick={() => handleTurnCompletion(false)} disabled={!isMyTurn} className="flex-1 inline-flex items-center justify-center gap-3 bg-yellow-600 hover:bg-yellow-500 text-white font-bold py-4 px-8 text-xl rounded-lg shadow-lg transition-transform transform hover:scale-105 disabled:bg-gray-600 disabled:cursor-not-allowed">
                        <SkipIcon />
                        Forfeit
                    </button>
                    <button onClick={() => handleTurnCompletion(true)} disabled={!isMyTurn} className="flex-1 inline-flex items-center justify-center gap-3 bg-[var(--color-primary)] text-white font-bold py-4 px-8 text-xl rounded-lg transition-all transform hover:scale-105 disabled:bg-pink-900/50 disabled:cursor-not-allowed disabled:transform-none"
                         style={isMyTurn ? { boxShadow: `0 0 15px var(--color-primary)`, filter: `drop-shadow(0 0 15px var(--color-primary))` } : {}}>
                        <CheckIcon />
                        Completed!
                    </button>
                </div>
